...
from optionalyzer.bs_logger import logger
...
class OptionChain:
    ...
    def __resolve_tickers(self):
        if self.ticker.lower() in ["nifty", "^nsei", "nsei"]:
            ...
            logger.debug("Resolved ticker to nifty-bank.")
            self.ticker = "nifty-bank"
        else:
            logger.error("Invalid ticker. Only nifty and nifty-bank are supported.")
            raise ValueError("Invalid ticker. Only nifty and nifty-bank are supported.")

    def __get_spot_price(self):
        if self.ticker == "nifty":
            logger.debug("Getting spot price of nifty.")
            ticker = yfinance.Ticker("^NSEI")
        elif self.ticker == "nifty-bank":
            logger.debug("Getting spot price of nifty-bank.")
            ticker = yfinance.Ticker("^NSEBANK")
        else:
            logger.error("Invalid ticker. Only nifty and nifty-bank are supported.")
            raise ValueError("Invalid ticker. Only nifty and nifty-bank are supported.")

        logger.debug("Getting spot price.")
    ...
    def option_chain(self, expiry_date, limit=10):
        ...
        """
        logger.info("Calling `self._chain`.")
        chain = self._chain(expiry_date=expiry_date)

        logger.debug("Limiting the option chain.")
    ...
    def calculate_iv(self, expiry_date, option_type="call", verbose=1):
        ...
        """
        if self.__option_chain is None:
            logger.info("Option chain not available. Scraping it now.")
            if verbose:
        ...
        for i, row in self.__option_chain.iterrows():
            logger.debug(
                f"Calculating IV for {option_type} at strike {row['Strike']} which is {i}th entry."
            )
        ...
        if option_type.lower() == "call":
            logger.debug("Adding Call IV to option chain.")
            self.__option_chain["Call IV"] = ivs
            self.call_iv = median_iv
            return median_iv
        else:
            logger.debug("Adding Put IV to option chain.")
